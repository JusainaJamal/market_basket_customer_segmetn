...
def predict_segment(recency, frequency, monetary):
    ...
    new_RFM_values = [[recency, frequency, monetary]]
...
def app():
    ...
    retail = pd.read_csv('C:\\Users\\USER\\Documents\\customer_project\\CleanRetailData.csv') 
    ...
    def cluster_plot(data_frame):
        fig = px.scatter_3d(data_frame, x = 'Recency', y='Frequency', z='Monetary',
                  color='RFM_Loyalty_Level', opacity = 0.8, width=600, height=600, template="plotly_dark")
...
# %%
    if page == "Customer Segmentation based on RFM Analysis":
        ...
        try:                   
            ...
            <h4 style="color:black;text-align:center;">Identified Clusters</h4>
            </div>
            """
            st.markdown(html_temp_title, unsafe_allow_html=True)
            cluster_plot(df)
            ...
            # Pie charts
            rfm_stats = df[["RFM_Loyalty_Level","RFMScore", "Recency", "Frequency", "Monetary"]].groupby("RFM_Loyalty_Level").agg(["mean"])
            ...
            with col1:
                ...
                <h5 style="color:black;text-align:center;">Recency</h5>
                </div>
                """
                st.markdown(html_temp_title, unsafe_allow_html=True)
            ...
            with col3:
                html_temp_title = """
                <div style="background-color:lightblue;padding:4px">
                <h5 style="color:black;text-align:center;">Frequency</h5>
                </div>
                """
                st.markdown(html_temp_title, unsafe_allow_html=True)
            ...
            with col5:
                html_temp_title = """
                <div style="background-color:lightblue;padding:4px">
                <h5 style="color:black;text-align:center;">Monetary</h5>
                </div>
                """
                st.markdown(html_temp_title, unsafe_allow_html=True)
...
# %%
    if page == "Predict Customer Cluster":
        ...
        if st.button('Predict Segment'):
            # Predict segment and display result
            predicted_segment = predict_segment(recency, frequency, monetary)
        
        # Map numeric label to loyalty level
        # loyalty_level = loyalty_mapping.get(predicted_segment, 'Unknown')
        
            st.write('Predicted Segment:', predicted_segment)
            # st.write('Loyalty Level:', loyalty_level)
